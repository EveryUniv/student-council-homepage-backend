...
class BusServiceTest {
    ...
    void listBusArrivalNoCached() {
        // given
        String stationId = BusStation.DKU_GATE.getNodeId();
        Instant now = Instant.now(clock);
        List<BusArrival> arrivals = BusArrivalMock.createList(5);
        CachedBusArrivals cached = new CachedBusArrivals(now, arrivals);
        when(memoryRepository.cacheArrivals(eq(stationId), any(), any())).thenReturn(cached);
    ...
    void listBusArrivalCached() {
        ...
        List<BusArrival> arrivals = BusArrivalMock.createList(5);
        CachedBusArrivals cached = new CachedBusArrivals(now, arrivals);
        when(memoryRepository.getArrivals(stationId, now)).thenReturn(cached);
